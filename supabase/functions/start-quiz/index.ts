...
serve(async (req) => {
  ...
  if (error || !rows?.length) return err("Could not load questions", 500);
  ...
  if (sessErr) return err("Session error", 500);

  return json({ sessionId: session.id, questions: shuffled, category, difficulty });
});